        ...
        **kwargs
        ...
        Args:
            reward_function: Function that computes reward(tweet, reply)
                           (optional if environment provided)
            reference_model: Frozen reference model (prevents drift)
            grpo_config: GRPO configuration
            environment: Optional TwitterReplyEnvironment for cleaner interface
            *args, **kwargs: Standard Trainer arguments
        """
        super().__init__(*args, **kwargs)
        
        # Support both direct reward function and environment
        self.environment = environment
        if environment is not None:
            self.reward_function = environment.compute_reward
            logger.info("Using environment for reward computation")
        elif reward_function is not None:
            self.reward_function = reward_function
        else:
            raise ValueError("Must provide either reward_function or environment")
